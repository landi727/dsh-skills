...
原则：
- 简报是事实来源，但**允许为模型能力做必要适配**（如模型渲染不了长文字 → 拆分或换工具）。保持语义不变；任何影响结果的适配必须向用户披露。
- 去 AI 味分工：内容知识（AI 味清单/风格锚点）来自 visual-brief；本 skill 负责用**该模型的语法**表达负向约束。
- **未知模型 → 用 Profile 0（通用自然语言），不要猜用最接近的专属 profile**；只有确认目标模型的能力与语法后才切换到专属 profile。
- 版本时效：本 skill 写到的版本号与参数（--v、CFG 等）是撰写时的默认值；若目标模型刚发布新版本，先查官方文档再定参数，不照抄旧参数。

## Profile 0 · Generic Natural Language（未知模型默认）

- 模型未列、或无法确认其语法与能力时，一律用本 profile。
- 成段自然语言：主体 → 环境 → 风格 → 构图 → 光线 → 色彩 → 材质，负向约束用「Avoid: …」成句。
- 给画幅/清晰度建议，但不给专属参数。

## Profile 1 · Midjourney

- 句式：`[主体/场景] [动作状态] [风格锚点] [构图/视角/焦距] [光线] [色彩] [材质媒介] [细节] [参数]`
- 参数（当前默认，版本更新时以官方文档为准）：`--ar`（比例）、`--v`（版本）、`--s`（风格化，越低越贴文字；去 AI 味常用 `--s 100 --style raw`）、`--iw`（参考图权重，有参考图时提高，让参考压过模型惯性）、`--no`（负面词）
...
- 采样（当前默认，版本更新以官方文档为准）：steps / CFG 给保守默认（SD 系 CFG 4–7）；Flux 用低 CFG（1–3）与对应采样器，注意习惯差异。
