...
Keep every externally testable conclusion accurate while hiding unnecessary audit narration from the default report.
...
Verify the color relationship system separately. At full-frame scale, check role locations, area balance, and warm/cool/neutral allocation. At thumbnail scale, check that major masses remain separated and accents remain bounded. In grayscale, check the value ladder. In an adjacency or simplified flat-color view, check every important touching pair and its separator. Seek a counterexample to each global color rule and mark quantitative claims as measured, estimated, or unresolved. Do not let a color name or scene mood substitute for pairwise relationships.
...
For style_transfer, also check that each established rule has visible support, a stated transfer scope, and explicit exceptions. Separate source-specific observations from general rules and hypotheses. Test whether the reusable wording specifies treatment of new subject structure rather than retaining source nouns. Confirm that target content is classified as required, candidate, or incidental; every included secondary element has a semantic group, focal role, narrative-device role, density-cluster or permitted quiet-zone interruption, color role where relevant, object tier, layer role, and visibility priority; and excluded candidates do not leak into the prompt. Confirm that target mapping uses role-based layer grammar instead of literal source IDs or coordinates, and that protected identity cues remain exposed. Confirm that protected quiet-space floors, cluster/text limits, clean-subject floors, and overflow decisions are explicit. Confirm that the execution contract precedes material-rich nouns and that those nouns do not expand beyond the extracted lighting, material, line-volume, and detail-density ceilings. Confirm that palette mode follows the user's instruction and defaults to `preserve_source` when no color direction was supplied. Confirm that prompt rules come only from the shared system and loaded subtype modules. A source audit verifies observations, not generated outcomes; track generation validation independently using generation-validation.md.
