...
Never substitute one for another. Record source-audit and generation-validation status separately. Select checks by declared priorities and task-relevant risks; the lists below are an inventory, not an exhaustive mandatory test suite. For analysis/prompt-only requests, define criteria and leave generation status `not_run`; finish without producing test images. A supplied generated output permits comparison within the user's review request. New generation requires authorization and an available image tool.
...
Treat color adhesion as critical only when it breaks a declared, source-supported separation or readability criterion. Nearby hues, temperatures and values are warning evidence, not sufficient failure conditions. Preserve intended near-value hue contrast, atmospheric blending, monochrome organization and lost edges; grayscale merging alone does not fail the color system. A fragmented counter-color fails only when a required balance relationship is lost. Revise the specific broken relation before unrelated drawing rules.

Treat broadly uniform activity as a critical density-rhythm failure when the reference depends on distinct quiet and dense zones, even when content and palette match. For declared critical capacity criteria, fail meaningful loss of quiet-field continuity, overflowing cluster/text limits, unallocated content, over-subdivided clean masses or collapsed breathing gaps. Account for stated tolerances and estimate uncertainty before declaring numerical failure; all allocated candidates may be present when they fit. Content completeness never compensates for these failures; omission of a candidate item is acceptable when required by the capacity budget. Identify the largest spillover region and revise element count, contrast, scale, or placement there before changing unrelated rules.
...
Distinguish an observed source estimate from a chosen acceptance bound. Record measurement status and a supported tolerance/range, or use an observable qualitative condition. Do not invent a universal tolerance. When a candidate lies near an uncertain threshold, re-inspect only that relationship or report partial/unresolved instead of an automatic critical failure. Explicit exact user requirements retain their precision.

Match acceptance order to the ranked mechanisms in generation-blueprint-schema.md. Include required positive effects (shape exaggeration, marks, action, value masses or density), not just upper limits. Declare critical criteria before inspecting the candidate; do not add new generic gates after generation or waive failed hard requirements silently.
