...
5. **Provenance Control（来源控制，最重要的安全阀）**：正文只允许三类内容——**S**（Source-backed，用户明确表达）/ **E**（Editorial Synthesis，由用户已有两个以上观点组合出的上位关系）/ **F**（External Fact，查证补充的客观事实）；**N（New Claim，用户材料中不存在的新解释/判断）禁止出现**。Thesis、一级论点、段落核心判断、结尾必须逐项标注来源。
...
### Step 2 · Central Question + Argument Clustering（主导主线与论点收束）
- **Central Question（主导主线）**：这篇文章真正围绕的一条主线是什么？可以是一个问题、一个矛盾、一个核心判断、一个未解决的冲突。**允许多个独立观察存在，只要它们受到主线控制**；禁止为了拥有主线而发明用户不存在的观点。
- **Argument Clustering**：哪些观点属于同一个更大的论证？产出：核心命题、一级论点、二级判断、证据、观察、旁支（轻处理或舍弃，记台账）、疑问、推测、**Tension（内在矛盾，决定结尾）**。
- **来源标记**：Thesis、一级论点、Tension 必须标注 S（用户明确表达）或 E（两个以上用户观点的上位组合）；查证事实标 F。任何无法标注为 S/E/F 的内容即 N，禁止进入正文。
...
正式写作前完成文章级设计：
- **开头从哪里进入**——四种同等级入口，由材料决定：**强判断 / 具体场景 / 核心矛盾 / 真正的问题**（见 `references/editorial-reference.md`）；
- **中心问题何时出现**；
...
- 每处增/删/并/调/改都记入编辑台账，并标注来源类型（S/E/F；N 禁止）。

### Step 7 · Critic（四查 + 台账）
1. **观点保真查**：观点来源存在、无新增观点（**正文无 N 类内容**）、疑问没变结论、立场强度没变、"未表达"没被补写；Thesis / 一级论点 / 结尾均有 S 或 E 标记。
...
4. **Editorial Review（完成度查）**：全文真正讨论的问题清楚吗？每部分服务主线吗？观点有层级吗？证据支撑判断吗？文章持续向前推进吗？有明显旁枝吗？段落只是并列吗？抽象判断有具体材料吗？小标题承担论证吗？结尾完成文章形成的主要矛盾吗？
   **Over-editing Check（过度编辑查）**：是否反复使用同一个核心隐喻？是否每段都有"漂亮的扣题句"？是否人为制造过多首尾呼应？是否连续出现编辑创造的金句？是否把原本自然的犹豫整理成过度完整的逻辑？——删掉一两句"聪明话"后，文章是否反而更自然？（原则：**优秀编辑负责建立关系，不负责让每个关系都显得聪明。**）
