---
name: visual-brief
description: 视觉子系统入口：只回答"这次视觉任务到底要创作什么"，产出 8 字段 Visual Task Brief（Goal / Deliverable / Content / Audience & Context / Creative Intent / References / Constraints / Acceptance）。不承担视觉设计、风格复刻、视觉 QA 与模型语法（分别由 style-replication、model-adapter 负责）。
...
6. **References**：有哪些参考；每份参考分别承担什么职责。
