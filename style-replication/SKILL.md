...
- **85% 近邻**：保留大部分高层语法，但仍不复制任何受保护签名元素，不得重建一个可辨认的商业身份。
...
- 参考缺失：本 skill 的 Trigger 不成立——回 `visual-brief` 补 References 字段，或确认任务不需要风格复刻（直接走 model-adapter）。参考模糊：Stage 2 用最小追问补齐。
